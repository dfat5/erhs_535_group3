...
```{r}
#Univariate Models
linear_model <- function(df = Cmax, indep_variable, dose_int) {
  df <- df %>%
    filter(independent_var == "indep_variable") %>% 
    filter(dose_int == "dose_int") %>% 
    select(ELU, indep_measure) 
}

a <- linear_model(indep_variable = ICS, dose_int = BID)
a
