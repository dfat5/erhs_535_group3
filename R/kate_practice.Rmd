...
# BID <- na.omit(new_est_results$BID)
# QD <- na.omit(new_est_results$QD)
# Ind_var <- unique(new_est_results$independent_var)
# seg_data <- as.data.frame(cbind(BID, QD, Ind_var))
# 
# coef_plot + geom_segment(aes(data = seg_data, x = BID,
#                              xend = QD, y = Ind_var, yend = Ind_var))

```
#barbell plots or add geom_segment (x start and x end) 

#plot using beeswarm, the dependent variables (ELU and eSP across individual mice). many mouse replicates for each drug
